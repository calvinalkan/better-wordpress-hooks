	...
	trait BetterWpHooksFacade {
		...
		public static function __callStatic( string $method, array $parameters ) {
			...
			if ( ! is_callable( $callable ) ) {
				...
				throw new \BadMethodCallException( 'Method ' . get_class( $instance ) . '::' . $method . '() does not exist.' );
			}
		...
		public static function dispatch( $event_blueprint, ...$payload ) {
			...
			return $dispatcher->dispatch( $event_blueprint, self::unwrapIfMultidimensional( $payload ) );
		...
		public static function dispatchIf( bool $condition, ...$args ) {
			...
			if ( $condition === TRUE ) {
				...
				if ( ! self::isEventObject( $event_object ) ) {
					...
					throw new \BadMethodCallException('Doing it wrong: The Plugin facade is not meant to be used for conditional dispatching. You should use event objects instead');
