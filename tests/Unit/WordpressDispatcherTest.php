<?php
    ...
    use Tests\TestListeners\InvokeableListener;
    ...
    class WordpressDispatcherTest extends BetterWpHooksTestCase
    {
        ...
        /** @test */
        public function a_listener_can_be_an_invokeable () {

            $this->dispatcher->listen('event1', InvokeableListener::class);

            $this->dispatchAndAssertAction('event1', 'foo', [
                InvokeableListener::class.'@__invoke',
                'foo_handled',
            ]);

            $this->reset();

            $this->dispatcher->listen(EventFakeStub::class, InvokeableListener::class);

            $event = new EventFakeStub();
            $event->creator = 'calvin';
        ...
        public function a_closure_receives_an_array_payload_as_separated_arguments_when_dispatching_an_action()
        {
            ...
            $this->dispatchAndAssertClosure('event', ['foo', 'bar', 'biz'], [
                $closure, 'foobarbiz',
            ]);
        ...
        public function a_listener_can_handle_an_event_conditionally_based_on_runtime_decisions()
        {
            ...
            $this->dispatchAndAssertAction('event', 'foo', [
                ConditionalListener::class.'@foobar', 'foobar',
            ]);
        ...
        public function the_payload_is_not_changed_when_a_listener_does_not_listen_at_runtime()
        {
            ...
            $this->dispatcher->listen(FilterableEvent::class, [
                ListenerWithReturnConditional::class, 'bar',
            ]);
