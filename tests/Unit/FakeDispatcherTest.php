	...
	class FakeDispatcherTest extends TestCase {
		...
		public function testAssertDispatched() {
			...
			try {
				...
				$this->assertThat( $e, new ExceptionMessage( 'The expected ['. self::class_namespace .'] event was not dispatched.'
				) );
		...
		public function testAssertDispatchedWithClosure() {
			...
			$this->fake->assertDispatched( function ( EventFakeStub $event ) use ( $event_stub ) {
		...
		public function testAssertDispatchedWithCallbackInt() {
			...
			try {
				...
				$this->assertThat( $e, new ExceptionMessage( 'The expected ['. self::class_namespace .'] event was dispatched 2 times instead of 1 times.' ) );
			}
		...
		public function testAssertNotDispatched() {
			...
			try {
				...
				$this->assertThat( $e, new ExceptionMessage( 'The unexpected ['. self::class_namespace .'] event was dispatched.'
				) );
			}
		}
		...
		public function testAssertNotDispatchedWithClosure() {
			...
			try {
				...
				$this->fake->assertNotDispatched( function ( EventFakeStub $event ) use ( $event_stub ) {
